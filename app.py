...
@APP.route("/question")
def question():
    num, max_num = session["num"], session["max_num"]
    game_mode = session["game_mode"]
    targetList = modes[game_mode]
    possibilities = rand.sample(
        targetList,
        5 if 5 < len(targetList) else len(targetList),
    )
...
def start_game():
    if request.method == "GET":
        return "Try going to '/form' to submit form"
